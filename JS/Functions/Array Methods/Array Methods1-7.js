...
const mailComp = users.map(user => {
  return {
    email: user.email,
    companyName: user.company.name
  };
});
console.log(mailComp)
...
const zipCode = users.filter(user => user.address.zipcode.startsWith("5"));
console.log(zipCode)
...
const zipCode5id =  zipCode.map(zipId => zipId.id)
...
const cUsers = usersNames.filter(cname => cname.startsWith("C")); 
console.log(cUsers)

const namesStartingWithC = users //shortened version
  .map(user => user.name)
...
const isSouthChristy = users.every(user => user.address.city  === "South Christy");
console.log(isSouthChristy)//false
